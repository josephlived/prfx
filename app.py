    ...
    if run_whois:
        ...
        if not prefixes:
            ...
            with st.spinner("Running WHOIS-only validation..."):
                ...
                results = validate_whois_only_prefixes(prefixes, catalog, whois_client, progress_callback=report_progress)
            ...
            with st.expander("Mode Notes"):
                st.write("WHOIS-only TRUE results come only from exact or approved-alias matches in OrgName or NetName.")
                st.write("Very close but non-exact WHOIS matches are flagged as To Review and remain FALSE.")
